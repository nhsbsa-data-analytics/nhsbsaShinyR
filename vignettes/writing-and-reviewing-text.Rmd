...
Each markdown file is included in a modules UI function with the `include_dynamic_md` function. It uses the `includeMarkdown` function provided by `{shiny}` and the `{glue}` package to get a markdown file as HTML while interpolating dynamic values into the text. You would think there would be an Rmarkdown version of this, but it seems there is not!

Dynamic text is specified with custom delimiters - `£>` and `<£`. This is to avoid conflicts with the markdown syntax, and also to make it easier to see where dynamic text is used. The delimiters and the code they contain are replaced with the result of running the code.

For example, the markdown that creates the text for a [module for headline figures](https://github.com/nhsbsa-data-analytics/care-home-prescribing-2020-2023/blob/main/inst/markdown/01_headline_figures_1.md) in the Care Homes Report has markdown like below.

```{r, results='asis', echo=FALSE}
# The original text string
my_text <- glue::glue("
  In £>latest_figures$latest_fy<£, we estimated a monthly average of \\
  £>latest_figures$ch_monthly_pats<£ care home patients aged 65 years and over \\
  receiving prescriptions. They received an estimated \\
  £>latest_figures$ch_annual_items_m<£ million prescription items at a drug cost \\
  of ££>latest_figures$ch_annual_cost_m<£ million.
")

# Use gsub to find the entire delimited section (including delimiters)
# and wrap it with the HTML <strong> tag.
# The parentheses in the pattern create a "capture group".
# The \\1 in the replacement refers to the content of that captured group.
html_text <- gsub(
  pattern = "(£>.*?<£)",
...
# Wrap the entire result in <pre><code> tags to make it a code block
# and print it directly into the document.
cat(paste0("<pre><code>", html_text, "</code></pre>"))
```

Each of the four bold sections consist of an opening delimiter (`£>`), a variable specified by name and a closing delimiter (`<£`). The variable is a list item, so the full variable name is `latest_figures$latest_fy`, for example. The code in the markdown file is run when the app is started, and the result of the code replaces the delimited section with the value of the variable.
